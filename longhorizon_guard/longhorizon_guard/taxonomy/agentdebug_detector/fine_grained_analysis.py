...
"""
    ...
    async def call_llm(self, prompt: str) -> str:
        """Call LLM API (supports Cloudflare Workers AI, OpenAI/Groq endpoints, and Gemini Direct API)"""
        # Pace requests to respect free-tier rate limits (3.5s delay)
        import time
        if not hasattr(self, '_last_call_time'):
            self._last_call_time = 0.0
        elapsed = time.time() - self._last_call_time
        if elapsed < 3.5:
            await asyncio.sleep(3.5 - elapsed)
        self._last_call_time = time.time()
        model_name = self.config.get('model', '')
        base_url = self.config.get('base_url', '')
        is_gemini = 'gemini' in base_url.lower() or 'gemini' in model_name.lower()
        is_cloudflare = 'cloudflare' in base_url.lower() or '@cf/' in model_name.lower()
        is_groq = 'groq.com' in base_url.lower() or 'groq' in model_name.lower()

        if is_groq:
            estimated_tokens = len(prompt) // 4 + 500
            await _groq_rate_limit(estimated_tokens)
        ...
        if is_cloudflare:
            cf_acc = self.config.get('account_id') or os.environ.get('CLOUDFLARE_ACCOUNT_ID', '')
            url = f"https://api.cloudflare.com/client/v4/accounts/{cf_acc}/ai/run/@cf/meta/llama-3.3-70b-instruct-fp8-fast"
        ...
        max_attempts = max(self.config.get('max_retries', 3), 5)
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if attempt == max_attempts - 1:
                        ...
                        raise
                    await asyncio.sleep(3.5 * (attempt + 1))
